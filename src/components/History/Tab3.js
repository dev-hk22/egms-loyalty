...
class Tab3 extends Component {
	...
	constructor(props) {
		...
		this.state = {
			...
			frmDate: this.props.props.navigation.state.params.frmDate ? this.props.props.navigation.state.params.frmDate : moment().clone().locale('en').startOf('month').format("DD-MM-YYYY"),
			// toDate: moment().clone().startOf('month').format("DD-MM-YYYY"),
			toDate: this.props.props.navigation.state.params.toDate ? this.props.props.navigation.state.params.toDate : moment().locale('en').format('DD-MM-YYYY'),
			fromDateError: '',
			toDateError: '',
			distributorId: '',
			lResponseData: '',
	...
	callApi = () => {
		...
		formData.append('officerId', this.state.carpenterId);
					...
					<FlatList
						...
						renderItem={({ item, index }) => (
							<ListItem key={index} style={{ flexDirection: 'column', alignItems: 'flex-start', }}>
								<View style={{ flex: 1, flexDirection: 'row', }}>
									{/* <View style={{ flex: 0.9, }} > */}
										<Text style={{ alignSelf: 'flex-start', fontSize: 14, color: this.props.enableDarkTheme ? 'white' : 'black' }}>{strings('login.serial_No')}: {item.redeemHistoryCash.id}</Text>
									{/* </View> */}
									{/* <View style={{ flex: 0.1, flexDirection: 'row' }}>
										<Text style={{ fontSize: 14, color: 'green', paddingRight: 3,  }}></Text>
										<Text style={{ fontSize: 14, color: 'green' }}>{item.redeemHistoryCash.value}</Text>
									</View> */}
								</View>
								{/* <View style={{ flex: 0.1, flexDirection: 'row' }}>
									<View style={{ flex: 0.9, }} >
										<Text style={{ alignSelf: 'flex-start', fontSize: 14, color: this.props.enableDarkTheme ? 'white' : 'black' }}>Product Name : {item.redeemHistoryCash.product_name}</Text>
									</View>
								</View> */}
								<View style={{ flex: 1, marginTop : 8}} >
									<Text style={{ fontSize: 14, color: this.props.enableDarkTheme ? 'white' : 'black' }}>Verified At: {moment(item.redeemHistoryCash.distributor_redemption_date).format('LLL')}</Text>
								</View>
								{/* <View style={{ flex: 1, }} >
									<Text style={{ fontSize: 14, color: this.props.enableDarkTheme ? 'white' : 'black' }}>{strings('login.batch_no')} : {item.redeemHistoryCash.batch_id ? item.redeemHistoryCash.batch_id :'To be generated'}</Text>
								</View> */}
								{/* <View style={{ flex: 1 }}>
									<Text style={{ fontSize: 14, color: this.props.enableDarkTheme ? 'white' : 'black' }}>Dealer Scanned : {item.redeemHistoryCash.dealerName}</Text>
								</View> */}
								{/* <View style={{ flex: 1 }}>
									<Text style={{ fontSize: 14, color: this.props.enableDarkTheme ? 'white' : 'black', color:'green' }}>{strings('login.status')} : {item.redeemHistoryCash.sap_interface_flag ? item.redeemHistoryCash.sap_interface_flag : "Redeemed"}</Text>
								</View> */}
							</ListItem>
						)}
					...
					{this.props.languageControl == 'Urdu - (اردو)' ?
						...
						<Grid style={{ }}>
							<Col >
								...
								<Text style={{  fontSize: this.props.languageControl == 'Marathi - (मराठी)' ? 12 : 16 ,
								alignSelf:'center', fontWeight: 'bold', 
								color: this.props.enableDarkTheme ? 'white' : 'black' }}>
									{strings('login.report_history_fromDate')} : </Text>
							</Col>
							<Col style={{  }}>
								{/* <DatePicker
									date={this.state.frmDate}
...
export default connect(mapStateToProps, null)(Tab3)