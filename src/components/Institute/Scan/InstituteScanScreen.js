...
import { Alert, AsyncStorage, BackHandler, Dimensions, Platform, StyleSheet, View, Image, TouchableOpacity, StatusBar, Linking } from 'react-native';
...
class InstituteScanScreen extends React.Component {
		...
		if (!lResponseData) {
			...
			setTimeout(() => {
				Alert.alert(
					...
					[
						{ text: strings('login.OK'), onPress: () => { this.props.navigation.navigate('InstituteMainScreen') } },
					],
					{ cancelable: false }
				)
			}, 500);
...
export default connect(mapStateToProps, null)(InstituteScanScreen)