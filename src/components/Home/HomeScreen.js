...
class HomeScreen extends Component {
  constructor(props) {
    ...
    this.distributorId;
    this.monthList = [
      {id: '1', value: strings('login.homeScreen_january')},
      {id: '2', value: strings('login.homeScreen_feb')},
      {id: '3', value: strings('login.homeScreen_march')},
      {id: '4', value: strings('login.homeScreen_april')},
      {id: '5', value: strings('login.homeScreen_may')},
      {id: '6', value: strings('login.homeScreen_june')},
      {id: '7', value: strings('login.homeScreen_july')},
      {id: '8', value: strings('login.homeScreen_aug')},
      {id: '9', value: strings('login.homeScreen_sep')},
      {id: '10', value: strings('login.homeScreen_oct')},
      {id: '11', value: strings('login.homeScreen_nov')},
      {id: '12', value: strings('login.homeScreen_dec')},
    ];
    this.yearList = [];
    this.state = {
      isDrawerOpen: false,
  ...
  componentDidMount = () => {
    ...
    if (this.state.selectedMonthId) {
      let mnth = _.filter(this.monthList, {id: this.state.selectedMonthId})[0]
        .value;
  ...
  handleDatePicked = date => {
    ...
    if (moment(a).isAfter(b)) {
      ...
      this.forceUpdate();
      this.setState(
        {fromDateError: '', toDateError: '', frmDate: date, frmDatePass: date},
        () => {
          this.getAsyncData();
        },
      );
  ...
  handleDatePicked1 = date => {
    ...
    if (a < b) {
      ...
      this.setState(
        {toDate: date, toDateError: '', fromDateError: '', toDatePass: date},
        () => {
          this.getAsyncData();
        },
      );
  ...
  getDashboard = pFormData => {
    ...
    this.setState({loading: true});
    var lUrl = URL + 'getDashboardCarpenter';
    fetch(lUrl, {
      method: 'POST',
      headers: {
        Accept: 'application/json',
  ...
  _setMonth(month, monthList) {
    if (monthList) {
      ...
      this.setState({selectedMonthId: idForMonth}, () =>
        this._getDashboardData(),
      );
    }
  }
  ...
  checkAndNavigate = () => {
    if (utilities.checkDateFormat(this.state.frmDatePass)) {
      this.props.navigation.navigate('HistoryScreen', {
        frmDate: this.state.frmDatePass,
        toDate: this.state.toDatePass,
      });
    } else {
      this.props.navigation.navigate('HistoryScreen', {
        frmDate: moment(this.state.frmDatePass, 'DD-MM-YYYY').format(
          'DD-MM-YYYY',
        ),
        toDate: moment(this.state.toDatePass, 'DD-MM-YYYY').format(
          'DD-MM-YYYY',
        ),
      });
    }
  ...
  checkAndNavigate1 = () => {
    ...
    if (utilities.checkDateFormat(this.state.frmDatePass)) {
      console.log('1');
      this.props.navigation.navigate('CashBatchesScreen', {
        frmDate: this.state.frmDatePass,
        toDate: this.state.toDatePass,
      });
    } else {
        ...
        if (responseJson.status == 200) {
          ...
          if (responseJson.offersData.length > 0) {
            ...
            for (var i = 0; i < responseJson.offersData.length; i++) {
              ...
              this.setState({
                offer_images: [
                  ...this.state.offer_images,
                  responseJson.offersData[i].image,
                ],
                show_carousel: true,
              });
            }
          ...
          <Left style={{flex: 0.1}}>
            <TouchableOpacity
              ...
              <Icon
                type="FontAwesome"
                name="bars"
                style={{fontSize: 30, color: '#FFFFFF', paddingRight: 10}}
              />
            </TouchableOpacity>
          </Left>
          <Body style={{flex: 0.8, alignItems: 'center'}}>
            <Title style={{color: 'white', fontSize: 16}}>
              {strings('login.dashboard_title')}
            </Title>
          </Body>
        ...
        <StatusBar
          backgroundColor={
            this.state.userType == 0
              ? MyColors.distributorColor
              : MyColors.dealerColor
          }
          ...
          {!this.state.showFullImage ? (
              ...
              <ScrollView
                ...
                {this.state.show_carousel ? (
                  <View style={styles.carousalcontainer}>
                    {/* <TouchableOpacity onPress={ () => utilities.showToastMsg("hi") } > */}
                    {/* <BackgroundCarousel offer_images={this.state.offer_images} 
                            onPress={ () => console.log("hi")}
                            /> */}
                    <SliderBox
                      images={this.state.offer_images}
                      sliderBoxHeight={250}
                      width="100%"
                      autoplay
                      ...
                      onCurrentImagePressed={index => {
                        // utilities.showToastMsg(this.state.offer_images[index]);
                        console.log(this.state.offer_images[index]);
                        this.setState({
                          showFullImage: true,
                          show_single_image: index,
                        });
                      }}
                      resize={'center'}
                      paginationBoxStyle={{
                    ...
                    />
                    {/* </TouchableOpacity> */}
                    {/* <Image source={require('../../images/arzol_img1.jpeg')} style={{margin:10, padding:10, height: '100%', width: "100%" }} resizeMode="stretch" /> */}
                  </View>
                ) : (
                  <View></View>
                )}
                    ...
                    <Col>
                      <Card
                        style={{
                          ...
                          borderWidth: 2,
                          overflow: 'hidden',
                          backgroundColor: MyColors.dashboard_box1,
                          borderRadius: 10,
                          borderColor: MyColors.dashboard_text1,
                        }}>
                        <Row
                          style={{
                            justifyContent: 'center',
                          ...
                          <Icon2
                            ...
                            style={{
                              ...
                              width: 35,
                              height: 35,
                              color: MyColors.dashboard_text1,
                            }}
                          />
                        </Row>
                        <Row style={{justifyContent: 'center'}}>
                          <Text
                            style={{
                              textAlign: 'center',
                              fontSize:
                                this.props.languageControl == 'Tamil - (தமிழ்)'
                                  ? 12
                                  : 16,
                              fontWeight: 'bold',
                              color: MyColors.dashboard_text1,
                            ...
                            {strings('login.total_coupon_scan')}
                          </Text>
                        </Row>
                        <Row style={{justifyContent: 'center'}}>
                          <Text
                            style={{
                              ...
                              textAlign: 'center',
                              fontSize: 30,
                              color: this.props.enableDarkTheme
                                ? 'white'
                                : 'black',
                            }}>
                            {this.state.totalCouponsRedeemed}
                          </Text>
                        </Row>
                      </Card>
                    </Col>
            ...
            <View style={{flex: 1, backgroundColor: '#000'}}>
                ...
                <TouchableOpacity
                  ...
                  onPress={() =>
                    ...
                    RNFetchBlob.config({
                      ...
                      .fetch(
                        'GET',
                        this.state.offer_images[this.state.show_single_image],
                      )
                  ...
                  <Text style={{color: '#ffffff', fontSize: 20}}>
                    ...
                    <Icon1
                      type="FontAwesome"
                      name="share-alt"
                      style={{fontSize: 20, color: '#FFFFFF', paddingRight: 10}}
                    />{' '}
                    Share
                  </Text>
                </TouchableOpacity>
                ...
                <TouchableOpacity
                  ...
                  <Text style={{color: '#ff0000', fontSize: 20}}>
                    <Icon1
                      type="FontAwesome"
                      name="window-close"
                      style={{fontSize: 25, color: '#FF0000', paddingRight: 10}}
                    />{' '}
                    Close
                  </Text>
                </TouchableOpacity>
              ...
              <ReactNativeZoomableView
                ...
                <Image
                  source={{
                    uri: this.state.offer_images[this.state.show_single_image],
                  }}
                  style={{
                    marginTop: 10,
