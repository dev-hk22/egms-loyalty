...
import {  BackHandler, StyleSheet, View, ScrollView, TouchableOpacity, StatusBar } from 'react-native';
...
class NotificationScreen extends Component {
	...
	async getNotifications(pFormData) {
		...
		var lUrl = URL + 'getNotificationsCarpenter';
		await fetch(lUrl, {
			method: 'POST',
			headers: {
			...
			.then((responseJson) => {
				...
				console.log(responseJson.notifications);
			})
			.catch((error) => {
				alert(error)
			});
