...
class InScreen extends Component {
    constructor(props) {
        super(props);
        this.state = {
            mechanicId: "",
        ...
        fetch(lUrl, {
            ...
            .then((responseJson) => {
                ...
                if (responseJson.status == 200) {
                    this.setState({ inData: responseJson.in })
    ...
    render() {
        return (
            <ScrollView style={{ flex: 1, backgroundColor: this.props.enableDarkTheme ? 'black' : 'white' }}>
                ...
                {this.state.inData.length > 0 ? this.state.inData.map((data) => (
                        ...
                        {this.props.languageControl == 'Urdu - (اردو)' ?
                            <Grid>
                                <Row>
                                    ...
                                    <Col size={0.8} >
                                        <Text style={{ color: 'grey', fontSize: 18, textAlign: "right" }}>{`: ${strings('login.couponSrNo')}`}</Text>
                                    </Col>
                                </Row>
                                <Col>
                            ...
                            <Grid>
                                <Row>
                                    <Col size={0.22}>
                                        <Text style={{ color: 'grey', fontSize: 18 }}>{strings('login.couponSrNo')}:</Text>
                                    </Col>
                                    <Col>
...
export default connect(mapStateToProps, null)(InScreen)