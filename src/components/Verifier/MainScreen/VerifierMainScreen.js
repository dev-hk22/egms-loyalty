...
import { Alert, StatusBar, AsyncStorage, BackHandler, Dimensions, Platform, StyleSheet, View, TextInput, Image, TouchableOpacity, KeyboardAvoidingView } from 'react-native';
...
export default class VerifierMainScreen extends React.Component {
	...
	handleBackPress = () => {
		Alert.alert(
			...
			[
				{ text: strings('login.alertNo') },
				{ text: strings('login.alertYes'), onPress: () => BackHandler.exitApp() },
			],
			{ cancelable: false }
		);
		...
		if (Platform.OS == 'ios') {
			return (
				<Header style={{ backgroundColor: '#e43c22' }}>
					...
					<Body>
						<Title style={{ color: '#FFFFFF', paddingLeft: 20 }}>SeQR Loyalty Demo</Title>
					</Body>
					<Right>
						<Menu>
							<MenuTrigger>
