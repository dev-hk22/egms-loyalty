...
import { Alert, BackHandler, Dimensions, Platform, StyleSheet, View, Image, TouchableOpacity, StatusBar, Animated, ScrollView, Easing, PermissionsAndroid, Button } from 'react-native';
...
class CustomerScanScreen extends Component {
    ...
    async _callForAPIRedeem(e) {
        ...
        console.log(redeemType);
            ...
            await scanApiObj.redeemCustCoupon(formData,this.state.accesstoken);
        ...
        if (Platform.OS == 'ios') {
            return (
                <Header style={{ backgroundColor: this.state.userType == 2 ? MyColors.dealerColor : MyColors.distributorColor }}>
                    <Left style={{ flex: 0.5 }}>
                        <TouchableOpacity onPress={() => {this.state.isSuccess ? this._scanAgain() : this.props.navigation.navigate('CustomerHomeScreen')}}>
...
export default connect(mapStateToProps, null)(CustomerScanScreen)