...
class CustSideMenu extends Component {
  ...
  async getAsyncData() {
    await AsyncStorage.multiGet(['USERDATA', 'FCMTOKEN','ACCESSTOKEN'], (err, result) => {
      ...
      if (lData) {
         ...
         this.setState({
          name: lData.data.company_name ? lData.data.company_name : lData.data.shop_name, imageURL: lData.data.brand_logo, userType: lData.data.userType,
           menuList: [
            ...
            // { title: strings('login.sidemenu_paymentoptions'), nav1: 'PaymentDetailsScreen' },
            ...
            { title: strings('login.sidemenu_makeorder'), nav1: 'OrderMasterScreen' } ,
            { title: strings('login.sidemenu_orderhistory'), nav1: 'CustomerOrderHistoryScreen' } ,
        ...
        console.log("sidemenu usertype", this.state.userType);
      }
    });
  }
  ...
  async _callForLogoutAPI() {
    ...
    formData.append('authUserId', this.carpenterId);
  ...
  _renderHeader(item, expanded) {
    // console.log("=====header " , JSON.stringify(item , null,2))
    if (item.title == 'LOGOUT' || item.title == 'लॉग आउट' || item.title == 'প্রস্থান' || item.title == 'લૉગ આઉટ' || item.title == 'ಲಾಗ್ ಔಟ್' || item.title == 'बाहेर पडणे' || item.title == 'ପ୍ରସ୍ଥାନ କର' || item.title == 'ਲਾੱਗ ਆਊਟ' || item.title == 'வெளியேறு' || item.title == 'లాగ్అవుట్' || item.title == 'لاگ آوٹ' || item.title == 'Déconnecter' || item.title == 'Njia kutoka') {
      ...
      item.title == 'ORDER' || item.title == 'ऑर्डर' || item.title == 'ORDER' || item.title == 'অর্ডার' || item.title == 'ઓર્ડર' || item.title == 'ಆರ್ಡರ್' || item.title == 'ऑर्डर' || item.title == 'ଆର୍ଡର' || item.title == 'ਆਰਡਰ' || item.title == 'ஆர்டர்' || item.title == 'ఆర్డర్' || item.title == 'آرڈر' || item.title == 'AGIZO' ||
      item.title == 'ORDER HISTORY' || item.title == 'आदेश इतिहास' || item.title == 'HISTORIQUE DES COMMANDES' || item.title == 'অর্ডার ইতিহাস' || item.title == 'ઓર્ડર ઇતિહાસ' || item.title == 'ಆರ್ಡರ್ ಇತಿಹಾಸ' || item.title == 'ऑर्डर इतिहास' || item.title == 'ଆର୍ଡର ଇତିହାସ' || item.title == 'ਆਰਡਰ ਇਤਿਹਾਸ' || item.title == 'ஆர்டர் வரலாறு' || item.title == 'ఆర్డర్ చరిత్ర' || item.title == 'آرڈر کی تاریخ' || item.title == 'HISTORIA YA AGIZO'
                            ...
                            item.title == 'PRODUCTS' || item.title == 'उत्पादों' || item.title == 'DES PRODUITS' || item.title == 'পণ্য' || item.title == 'પ્રૉડક્ટ્સ' || item.title == 'ಪ್ರಾಡಕ್ಟ್ಸ್' || item.title == 'प्रॉडक्ट्स' || item.title == 'ଉତ୍ପାଦଗୁଡିକ' || item.title == 'ਪ੍ਰਾਡਕ੍ਟ੍ਸ' || item.title == 'தயாரிப்புகள்' || item.title == 'ఉత్పత్తులు' || item.title == 'مصنوعات' || item.title == 'Bidhaa' ?
                              ...
                              item.title == 'REPORT HISTORY' || item.title == 'रिपोर्ट इतिहास' || item.title == 'Historique du rapport' || item.title == 'ইতিহাসের প্রতিবেদন করুন' || item.title == 'ઇતિહાસની જાણ કરો' || item.title == 'ಇತಿಹಾಸವನ್ನು ವರದಿ ಮಾಡಿ' || item.title == 'इतिहास नोंदवा' || item.title == 'ରିପୋର୍ଟ ଇତିହାସ' || item.title == 'ਇਤਿਹਾਸ ਦੀ ਰਿਪੋਰਟ ਕਰੋ' || item.title == 'வரலாற்றைப் புகாரளிக்கவும்' || item.title == 'చరిత్రను నివేదించండి' || item.title == 'تاریخ(ہسٹری ) کی اطلاع دیں' || item.title == 'Ripoti ya historia' ?
                                ...
                                item.title == 'PRODUCTS HISTORY' || item.title == 'उत्पाद इतिहास' || item.title == 'HISTOIRE DES PRODUITS' || item.title == 'পণ্য ইতিহাস' || item.title == 'પ્રૉડક્ટ્સ હિસ્ટરી' || item.title == 'ಪ್ರಾಡಕ್ಟ್ಸ್ ಹಿಸ್ಟರೀ' || item.title == 'प्रॉडक्ट्स हिस्टरी' || item.title == 'ଉତ୍ପାଦ ଇତିହାସ |' || item.title == 'ਪ੍ਰਾਡਕ੍ਟ੍ਸ ਹਿਸ੍ਟਰੀ' || item.title == 'தயாரிப்புகள் வரலாறு' || item.title == 'ఉత్పత్తుల చరిత్ర' || item.title == 'مصنوعات کی تاریخ' || item.title == 'Historia ya Bidhaa' ?
                                  ...
                                  :
                                  item.title == 'ORDER' || item.title == 'ऑर्डर' || item.title == 'ORDER' || item.title == 'অর্ডার' || item.title == 'ઓર્ડર' || item.title == 'ಆರ್ಡರ್' || item.title == 'ऑर्डर' || item.title == 'ଆର୍ଡର' || item.title == 'ਆਰਡਰ' || item.title == 'ஆர்டர்' || item.title == 'ఆర్డర్' || item.title == 'آرڈر' || item.title == 'AGIZO' ?
                                    <Icon type="FontAwesome5" name="store" style={{ fontSize: 18, color: this.props.enableDarkTheme ? 'white' : 'black' }} />
                                    :
                                    item.title == 'ORDER HISTORY' || item.title == 'आदेश इतिहास' || item.title == 'HISTORIQUE DES COMMANDES' || item.title == 'অর্ডার ইতিহাস' || item.title == 'ઓર્ડર ઇતિહાસ' || item.title == 'ಆರ್ಡರ್ ಇತಿಹಾಸ' || item.title == 'ऑर्डर इतिहास' || item.title == 'ଆର୍ଡର ଇତିହାସ' || item.title == 'ਆਰਡਰ ਇਤਿਹਾਸ' || item.title == 'ஆர்டர் வரலாறு' || item.title == 'ఆర్డర్ చరిత్ర' || item.title == 'آرڈر کی تاریخ' || item.title == 'HISTORIA YA AGIZO' ?
                                      <Icon type="FontAwesome" name="history" style={{ fontSize: 18, color: this.props.enableDarkTheme ? 'white' : 'black' }} />
                                      :
  ...
  onPress2 = (lang) => {
    ...
    this.setState({
      menuList: [
        ...
        // { title: strings('login.sdemenu_scan'), nav1: 'CustomerScanScreen' },
        // { title: strings('login.sidemenu_couponhistory'), nav1: 'HistoryScreen' },
        // this.state.userType == 0 ? { title: strings('login.sidemenu_report'), nav1: 'ReportScreen' } : {},
        // this.state.userType == 0 ? { title: strings('login.sidemenu_reporthistory'), nav1: 'ReportHistory' } : {},
        // this.state.userType == 0 ? {} : { title: strings('login.sidemenu_paymentoptions'), nav1: 'PaymentDetailsScreen' },
        ...
        // { title: strings('login.logout') },
        // { title: "LANGUAGE" },
        { title: strings('login.profile'), nav1:  "CustomerProfileScreen" },
        // this.state.userType == 4 ?  { title: strings('login.sidemenu_paymentoptions'), nav1: 'PaymentDetailsScreen' } : {},
        this.state.userType == 2 ? { title: strings('login.sdemenu_scan'), nav1: 'DealerScanScreen' } : { title: strings('login.sdemenu_scan'), nav1: 'CustomerScanScreen' },
        this.state.userType == 2 ? { title: strings('login.sidemenu_couponhistory'), nav1: 'DealerHistoryScreen' } : { title: strings('login.sidemenu_couponhistory'), nav1: 'HistoryScreen' },
        // this.state.userType == 4 ? { title: strings('login.cashbash'), nav1: 'CashBatchesScreen' } : {},
        this.state.userType == 5 ? { title: strings('login.sidemenu_makeorder'), nav1: 'OrderMasterScreen' } : {},
        this.state.userType == 5 ? { title: strings('login.sidemenu_orderhistory'), nav1: 'CustomerOrderHistoryScreen' } : {},
        this.state.userType == 5 ? { title: strings('login.sidemenu_report'), nav1: 'ReportScreen' } : {},
        this.state.userType == 5 ? { title: strings('login.sidemenu_reporthistory'), nav1: 'ReportHistory' } : {},
